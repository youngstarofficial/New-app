...
let allBranches = [];
let allDistricts = [];
let currentStudents = []; // stores the currently displayed (filtered) rows
...
async function fetchStudents() {
  try {
    ...
    allBranches = data.allBranches.sort();
    allDistricts = data.allDistricts.sort();
    allStudents = data.students;

    currentStudents = [...allStudents]; // initialize
...
function populateTable(students, selectedCaste = "") {
  currentStudents = [...students]; // always store what's displayed
...
function applyFilters() {
  ...
  const casteToFilter = studentCasteInput || filterCaste; // same logic used elsewhere
  ...
  if (casteToFilter && categoryMap[casteToFilter]) {
    ...
    // sort ascending by the selected caste numeric field
    filtered.sort((a, b) => {
    ...
    // fallback sorting by instituteCode so results are deterministic
...
// Download PDF with Student Info + Filtered Results sorted by rank (selected caste)
function downloadPDF() {
  ...
  // Table headers (only the 6 fields you requested)
  const headers = [
    "Institute Code",
    "Institute Name",
    "Place",
  ...
  // Use the rows currently displayed (currentStudents) and sort them by selected caste field (if any)
  ...
  if (studentCaste && categoryMap[studentCaste]) {
    ...
    // fallback: sort by instituteCode
    rowsSource.sort((a, b) => (a.instituteCode || "").localeCompare(b.instituteCode || ""));
  }
